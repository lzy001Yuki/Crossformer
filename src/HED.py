...
from TSALayer import Attention, AttentionLayer, TwoStageAttention
...
class scale_block(nn.Module):
    ...
    def __init__(self, win_size, d_model, n_heads, d_ff, depth, dropout, \
        ...
        for i in range(depth):
            self.encode_layers.append(TwoStageAttention(seg_num, factor, d_model, n_heads, \
                                                             d_ff, dropout))
...
class DecoderLayer(nn.Module):
    def __init__(self, seg_len, d_model, n_heads, d_ff=None, dropout=0.1, out_seg_num=10, factor=10):
        ...
        self.self_attention = TwoStageAttention(out_seg_num, factor, d_model, n_heads, \
                                                         d_ff, dropout)
...
class Decoder(nn.Module):
    ...
    def forward(self, x, cross):
        ...
        final_predict = rearrange(final_predict, 'b (out_d seg_num) seg_len -> b (seg_num seg_len) out_d',
                                      out_d=ts_d)
